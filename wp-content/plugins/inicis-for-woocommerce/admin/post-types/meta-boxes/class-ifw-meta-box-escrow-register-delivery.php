...
class IFW_Meta_Box_Escrow_Register_Delivery {
    public static function output( $post ) {
        ...
        //에스크로 실시간 계좌이체 관리자 배송등록 및 환불 가능 상태 확인
        $status_method_arr = get_option('inicis_pg_inicis_stdescrowbank_possible_register_delivery_info_status_for_admin');
        $status_method_arr = explode(',', $status_method_arr);
        ...
        <script type="text/javascript">
            ...
            function onClickCancelRequest(){
                if(confirm("정말 환불 처리하시겠습니까?\n\n처리 이후에 이전 상태로 되돌릴 수 없습니다. 신중하게 선택해주세요.")) {
                    var data = {
                        action: "inicis_stdescrow_bank_order_cancelled",
                        post_id: '.$post->ID.',
            ...
            function checkReceipt(){
                window.open("https://iniweb.inicis.com/app/publication/apReceipt.jsp?noMethod=1&noTid='.get_post_meta($post->ID, '_inicis_paymethod_tid', true).'");
            } 
        </script>'; ?>
        ...
        <script type="text/javascript">
                ...
                if(confirm("배송 정보를 등록하시겠습니까?")){
                    if(jQuery("#_order_shipping_writer").val() != '' && jQuery("#_order_shipping_company").val() != '' && jQuery("#_order_tracking_number").val() != '') { 
                        ...
                        jQuery.ajax({
                            type:'POST',
                            dataType:'text',
                            url: '<?php echo home_url().'/wc-api/WC_Gateway_Inicis_StdEscrow_bank?type=delivery'; ?>',
                            data: {
                                postid: '<?php echo $post->ID; ?>',
                                tid: '<?php $tmp = get_post_meta($post->ID, '_inicis_paymethod_tid', true); echo $tmp; ?>', //거래번호
                                oid: '<?php $tmp = get_post_meta($post->ID, '_txnid', true); echo $tmp; ?>', //주문번호
                                ...
                                recvName :'<?php echo ifw_get($orderinfo, 'billing_first_name'); ?>', //수신자 이름
                                ...
                                recvTel :'<?php $tmp = get_post_meta($post->ID, '_billing_phone', true); echo preg_replace("/[^0-9]*/s", "", $tmp); ?>', //수신자 전화번호
                                goodsCode :'<?php echo ifw_get($orderinfo, 'id'); ?>', //상품코드 (선택), 여기서는 주문ID(포스트ID)값을 임의로 입력
                                ...
                                reserved2 :'', //상품옵션2(선택)
                                reserved3 :'', //상품옵션3(선택)
                                mid : '<?php $tmp = get_post_meta($post->ID, '_payment_method', true); echo get_option('inicis_pg_escrow_merchant_id'); ?>'  //이니페이 에스크로 상점 아이디값 (필수)
                            },
            ...
            function register_escrow_once(){
                jQuery.ajax({
                    type:'POST',
                    dataType:'text',
                    url: '<?php echo home_url().'/wc-api/WC_Gateway_Inicis_StdEscrow_bank?type=delivery_okay'; ?>',
                    data: {
        ...
        <?php   
        if( in_array($orderinfo->get_status(), $status_method_arr) && (get_post_meta($post->ID, '_payment_method', true) == 'inicis_stdescrow_bank') ){
            $shipping_num = get_post_meta($post->ID, '_inicis_escrow_shipping_number', true);
            $order_cancelled = get_post_meta($post->ID, '_inicis_escrow_order_cancelled', true);
            $delivery_add = get_post_meta($post->ID, '_inicis_paymethod_escrow_delivery_add', true);
            ...
            if(!empty($order_cancelled) && $delivery_add == 'yes') {
                ...
                if(!empty($delivery_add) && $delivery_add == 'yes') {
                    ...
                    } else if($orderinfo->get_status() == 'cancel-request' || $orderinfo->get_status() == 'cancelled'){
                ...
                if($delivery_add == 'yes' && $orderinfo->get_status() == "cancel-request"){
