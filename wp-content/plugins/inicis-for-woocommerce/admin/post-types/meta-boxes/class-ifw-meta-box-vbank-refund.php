...
class IFW_Meta_Box_Vbank_Refund {
    public static function output( $post ) {
		...
		$tid = get_post_meta( ifw_get($order, 'id'), '_inicis_paymethod_tid', true);
        $vbank_refund_add = get_post_meta( ifw_get($order, 'id'), '_inicis_paymethod_vbank_add', true);
        $vbank_noti_received = get_post_meta( ifw_get($order, 'id'), '_inicis_vbank_noti_received', true);

        $url = admin_url('post.php?post='.ifw_get($order, 'id').'&action=edit');
...
?>
        ...
        <script type="text/javascript">
            ...
            function checkReceipt(){
                window.open("https://iniweb.inicis.com/app/publication/apReceipt.jsp?noMethod=1&noTid=<?php echo get_post_meta($post->ID, '_inicis_paymethod_tid', true); ?>");
            }
                ...
                if(confirm("환불 정보를 등록하시겠습니까?")) {
                    if (jQuery("#_order_refund_bankcode").val() != "-1" && jQuery("#_order_refund_vacc_num").val() != "" && jQuery("#_order_refund_vacc_name").val() != "" && jQuery("#_order_refund_reason").val() != "") {
                        ...
                        jQuery.ajax({
                            type: 'POST',
                            dataType: 'text',
                            url: '<?php echo home_url().'/wc-api/WC_Gateway_Inicis_StdVbank?type=vbank_refund_add'; ?>',
                            data: {
                ...
                if(confirm("환불 정보를 수정하시겠습니까?")) {
                    if (jQuery("#_order_refund_bankcode").val() != "-1" && jQuery("#_order_refund_vacc_num").val() != "" && jQuery("#_order_refund_vacc_name").val() != "" && jQuery("#_order_refund_reason").val() != "") {
                        ...
                        jQuery.ajax({
                            type: 'POST',
                            dataType: 'text',
                            url: '<?php echo home_url().'/wc-api/WC_Gateway_Inicis_StdVbank?type=vbank_refund_modify'; ?>',
                            data: {
        ...
        <?php
        $vbank_refund_bankcode = get_post_meta($post->ID, '_vbank_refund_bankcode', true);
        $vbank_refund_vaccnum = get_post_meta($post->ID, '_vbank_refund_vaccnum', true);
        $vbank_refund_vaccname = get_post_meta($post->ID, '_vbank_refund_vaccname', true);
        $vbank_refund_reason = get_post_meta($post->ID, '_vbank_refund_reason', true);
        ...
        function isDisabled(){
            global $post;
            $vbank_refunded = get_post_meta($post->ID, '_inicis_paymethod_vbank_refunded', true);
