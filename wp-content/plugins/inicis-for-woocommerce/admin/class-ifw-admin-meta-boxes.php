...
class IFW_Admin_Meta_Boxes {
	...
	public function __construct() {
        ...
        add_action( 'inicis_payment_add_metabox', array( $this, 'add_other_meta'), 10, 1);
	...
	public function add_meta_boxes() {
        ...
        if(isset($_GET['post'])) {
            if (get_post_meta($_GET['post'], '_payment_method', true) == 'inicis_stdescrow_bank') {

                $payment_method = get_post_meta($_GET['post'], '_payment_method', true);

                //관리자 배송정보 등록/환불 가능 주문상태 지정
                $tmp_setting = get_option('inicis_pg_inicis_stdescrowbank_possible_register_delivery_info_status_for_admin');
                $refund_mypage_status = explode(',', $tmp_setting);
            ...
            } else if( get_post_meta($_GET['post'], '_payment_method', true) == 'inicis_stdvbank' ) {
                add_meta_box(
                ...
                do_action( 'inicis_payment_add_metabox', $_GET['post'] );
            }
        }
	}

    public function add_other_meta($post) {
        if( in_array( get_post_meta($post, '_payment_method', true), apply_filters('inicis_other_method_meta', array('inicis_stdcard', 'inicis_stdbank', 'inicis_stdhpp', 'inicis_stdkpay', 'inicis_stdvbank', 'inicis_stdescrow_bank', 'inicis_stdsamsungpay' ) ) ) ) {
