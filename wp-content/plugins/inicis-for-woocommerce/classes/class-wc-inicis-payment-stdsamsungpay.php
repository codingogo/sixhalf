	...
	if ( ! class_exists( 'WC_Gateway_Inicis_StdSamsungpay' ) ) {
		...
		class WC_Gateway_Inicis_StdSamsungpay extends WC_Gateway_Inicis {
			public function __construct() {
				...
				$this->method_title = __('삼성페이', 'inicis_payment');
				...
				$settings = inicis_pg_get_setting( 'stdsamsungpay' );
//				$this->init_settings();
				...
				if( empty($this->settings['title']) ){
					$this->title =  __('삼성페이 결제', 'inicis_payment');
					$this->description = __('삼성페이를 통해 결제를 진행합니다.', 'inicis_payment');
				}else{
			...
			function init_action() {
				...
				add_action( 'wp_ajax_repay_request_' . $this->id, array( &$this, 'wp_ajax_repay_request' ) );
			}

            function thankyou_page( $order_id ) {
                echo __('<div id="inicis_thankyou_text"><p>삼성페이로 결제되었습니다. 감사합니다.</p></div>', 'inicis_payment');
            }
			...
			function init_form_fields() {
				...
				$this->form_fields = array_merge($this->form_fields, array(
					...
					'nointerest' => array(
						...
						'default' => '',
						'description' => __('pgall.co.kr 사이트의 친절한 메뉴얼을 참고하여 입력하시기 바랍니다. 무이자 할부는 이니시스 계약시 무이자 할부 계약이 체결되어 있어야 합니다. 단, 최소 결제금액이 5만원 이상인 경우에만 할부 결제가 허용됩니다. 무이자 할부 허용하시면 무이자 할부에 따른 수수료는 상점에서 부담하게 됩니다. 수수료는 이니시스에 문의하여 주십시오. (단, 이니시스에서 모든 가맹점을 대상으로 하는 무이자 이벤트인 경우는 제외입니다)', 'inicis_payment'),
						'desc_tip' => true, 
						), 
                    'cardpoint' => array(
		...
		if ( defined('DOING_AJAX') ) {
			$ajax_requests = array('payment_form_inicis_stdsamsungpay', 'refund_request_inicis_stdsamsungpay', 'repay_request_inicis_stdsamsungpay');
			if( in_array( $_REQUEST['action'], $ajax_requests ) ){
				new WC_Gateway_Inicis_StdSamsungpay();
			}
		}	
	}
