	...
	if ( ! class_exists( 'WC_Gateway_Inicis_StdEscrow_bank' ) ) {
	    ...
	    class WC_Gateway_Inicis_StdEscrow_bank extends WC_Gateway_Inicis{
	        public function __construct(){
	            ...
	            $this->method_title = __('에스크로 계좌이체 (웹표준)', 'inicis_payment');
                ...
                $settings = inicis_pg_get_setting( 'stdescrowbank' );
//				$this->init_settings();
				...
				if( empty($this->settings['title']) ){
					...
					$this->description = __('이니시스 결제대행사를 통해 결제합니다. 에스크로 결제의 경우 인터넷익스플로러(IE) 환경이 아닌 경우 사용이 불가능합니다. 결제 완료시 내 계정(My-Account)에서 주문을 확인하여 주시기 바랍니다.', 'inicis_payment');
				}else{
            ...
            function ajax_inicis_escrow_bank_order_cancelled() {
                ...
                {
                    $get_shipping_num_send = get_post_meta($_POST['post_id'], "_inicis_paymethod_escrow_delivery_add", true);
            ...
            function inicis_escrow_order_shipping_save( $post_id, $post ) {
                if ( isset( $_POST['_order_tracking_number'] ) ) {
                    update_post_meta($post->ID, '_inicis_escrow_shipping_number', $_POST['_order_tracking_number']);
                    update_post_meta($post->ID, '_inicis_escrow_shipping_number_time', current_time( 'mysql' ));
                }
            }

            function thankyou_page( $order_id ) {
                echo __('<div id="inicis_thankyou_text"><p>이니시스 에스크로 계좌이체로 결제되었습니다. 감사합니다.</p></div>', 'inicis_payment');
            }
            ...
            function inicis_escrow_request_cancel_before_confirm($posted){
                ...
                if($resultCode == '00'){
                    ...
                    //환불처리시 변경될 상태
                    $refunded_status = get_option('inicis_pg_inicis_stdescrowbank_order_status_after_refund');
                    $orderinfo->update_status( $refunded_status );  //취소처리완료 상태로 변경

                    update_post_meta($postid, '_inicis_escrow_order_cancelled', TRUE);
                    update_post_meta($postid, '_inicis_escrow_order_cancelled_time', current_time( 'mysql' ));
            ...
            function inicis_escrow_cancel_request_by_user($orderid){
                ...
                $mid = get_option('inicis_pg_escrow_merchant_id');

                $tid = get_post_meta($orderid, '_inicis_paymethod_tid', true);
                $isescrow = get_post_meta($orderid, '_payment_method', true);

                if($isescrow == 'inicis_stdescrow_bank'){
                    $data = array('mid' => $mid,
                                  'tid' => $tid,
                                  'msg' => '',
                                  'postid' => $orderid);
            ...
            function inicis_escrow_cancel_request_by_admin($post_id){
                ...
                $tid = get_post_meta($post_id, '_inicis_paymethod_tid', true);
                $isescrow = get_post_meta($post_id, '_payment_method', true);

                $delivery_register_name = get_option('inicis_pg_inicis_stdescrowbank_delivery_register_name'); //배송정보 등록자명

                if($isescrow == 'inicis_stdescrow_bank'){
                    $data = array('mid' => $mid,
                                  'tid' => $tid,
                                  'dcnf_name' => $delivery_register_name,   //구매거절 확인자명
                                  'postid' => $post_id);
            ...
            function inicis_escrow_delivery_add($posted) {
                ...
                if ( isset( $_POST['invoice'] ) ) {
                    update_post_meta($_POST['postid'], '_inicis_escrow_shipping_number', $_POST['invoice']);
                    update_post_meta($_POST['postid'], '_inicis_escrow_shipping_number_time', current_time( 'mysql' ));
                } else {
                    return;
                }
            ...
            function inicis_escrow_delivery_okay($posted){
                global $woocommerce;
                $orderinfo = new WC_Order($_POST['postid']);
                $orderinfo->add_order_note( __('판매자님께서 고객님의 에스크로 결제 주문을 배송 등록 또는 수정 처리하였습니다.','inicis_payment') );
                $delivery_tmp = get_post_meta( $_POST['postid'], '_inicis_paymethod_escrow_delivery_add', true );
                if( empty( $delivery_tmp ) ) {
                    $payment_method = get_post_meta($_POST['postid'], '_payment_method', true);

                    $after_enter_shipping_number_status = get_option('inicis_pg_inicis_stdescrowbank_order_status_after_enter_shipping_number');  //관리자 배송정보 등록/환불 가능 주문상태 지정
                    $orderinfo->update_status( $after_enter_shipping_number_status );
                }

                //1회라도 에스크로 배송 등록을 한 경우 표시(배송 수정으로 변경되도록)
                update_post_meta($_POST['postid'], "_inicis_paymethod_escrow_delivery_add", "yes");  //에스크로 배송 정보 최초 추가 확인값 추가
                update_post_meta($_POST['postid'], '_inicis_paymethod_escrow_delivery_add_time', current_time( 'mysql' ));
                do_action('inicis_escrowbank_delivery_add');
                echo "success";
                die();
            }
            ...
            function init_form_fields() {
                ...
                $this->form_fields = array_merge($this->form_fields, array(
                    ...
                    'delivery_sender_phone' => array(
                        ...
                        'type' => 'text',
                        'description' => __('배송정보 등록시 사용되는 발신자의 전화번호로 \'<strong>000-0000-0000</strong>\'과 같이 입력해주시면 됩니다.', 'inicis_payment'),
            ...
            function inicis_escrow_mypage_cancell_request($order) {
                ...
                $pay_escrow = get_post_meta( ifw_get($orderinfo, 'id'), '_payment_method', true);
                if(empty($pay_escrow) && $pay_escrow != "inicis_stdescrow_bank") { return; }

                $tmp_setting = get_option('inicis_pg_inicis_stdescrowbank_possible_refund_status_for_mypage');
                $valid_order_status = explode(',', $tmp_setting);

                if( in_array($orderinfo->get_status(), $valid_order_status) ){
                ...
                echo '<h2>주문 취소</h2>
                    ...
                    <form name=escrow_bank_ini method=post action="'.home_url().'/wc-api/'.get_class($this).'?type=cancelled">
                        <input type="hidden" name="oid" id="oid" value="'.ifw_get($orderinfo, 'id').'"/>
                        <input type="hidden" name="tid" id="tid" value=""/>
                        <input type="hidden" name="mid" id="mid" value=""/>
                        <input type="hidden" name="msg" id="msg" value=""/>
                        <input type="button" class="button" name="button_cancel_order_request" value="'.__('주문 취소','inicis_payment').'" onClick="javascript:OnEscrowCancelConfirmBtn('.ifw_get($orderinfo, 'id').');"/>';
            ...
            function inicis_escrow_mypage_accept_request($order) {
                ...
                $tmp_setting = get_option('inicis_pg_inicis_stdescrowbank_possible_check_and_reject_status_for_customer');
                if($tmp_setting == false) { return; }
                $status_method_arr = explode(',', $tmp_setting);

                if(count($status_method_arr) == 1 && empty($status_method_arr[0])) {
                    $status_method_arr = array(__('shipped','inicis_payment'));    }

                if( get_post_meta($order, '_inicis_escrow_shipping_number', true) != "" ) {
                    //이니페이 설정 페이지 옵션 값에서 가져오기
                    $delivery_shipping_num = get_post_meta( ifw_get($orderinfo, 'id'), '_inicis_escrow_shipping_number', true);

                    $delivery_company_name      = get_option('inicis_pg_inicis_stdescrowbank_delivery_company_name');       //택배사명
                    $delivery_register_name     = get_option('inicis_pg_inicis_stdescrowbank_delivery_register_name');      //배송정보 등록자명
                    $delivery_sender_name       = get_option('inicis_pg_inicis_stdescrowbank_delivery_sender_name');        //송신자명
                    $delivery_sender_postnum    = get_option('inicis_pg_inicis_stdescrowbank_delivery_sender_postnum');     //송신자 우편번호
                    $delivery_sender_addr1      = get_option('inicis_pg_inicis_stdescrowbank_delivery_sender_addr1');       //송신자 기본주소
                    $delivery_sender_addr2      = get_option('inicis_pg_inicis_stdescrowbank_delivery_sender_addr2');       //송신자 상세주소
                    $delivery_sender_phone      = get_option('inicis_pg_inicis_stdescrowbank_delivery_sender_phone');       //송신자 전화번호

                    echo '<h2>배송정보</h2><p>배송업체 : '.$delivery_company_name.'</br>송장번호 : '.$delivery_shipping_num.'</p>';
                }

                if( in_array($orderinfo->get_status(), $status_method_arr) && (get_post_meta( ifw_get($orderinfo, 'id'), '_payment_method', true) == 'inicis_stdescrow_bank') ){
                    if ( get_post_meta( ifw_get($orderinfo, 'id'), '_inicis_escrow_order_confirm') == TRUE ){
                        echo '<h2>'.__('구매 확인/거절','inicis_payment').'</h2>';
                        echo '<p class="order-info">'.__('구매 확인이 완료되었습니다.','inicis_payment').'</p>';
                    } else if (get_post_meta( ifw_get($orderinfo, 'id'), '_inicis_escrow_order_confirm_reject') == TRUE) {
                        echo '<h2>'.__('구매 확인/거절','inicis_payment').'</h2>';
                        ...
                        <input type="hidden" name="accept_order" id="accept_order" value="'.ifw_get($orderinfo, 'id').'"/>';
                        echo '<input type="button" class="button" name="button_refund_request" value="'.__('구매 확인/거절','inicis_payment').'" onClick="javascript:OnConfirmBtn('.ifw_get($orderinfo, 'id').');"/>';
            ...
            function inicis_escrow_request_confirm($posted){
                ...
                if($resultCode == "00"){
                    ...
                    if($iniescrow->GetResult("CNF_Date") != ""){
                        ...
                        update_post_meta($postid, '_inicis_escrow_order_confirm_time', current_time( 'mysql' ));
                        ...
                        do_action('inicis_escrowbank_confirm_yes');
                    } else {
                        ...
                        update_post_meta($postid, '_inicis_escrow_order_confirm_reject_time', current_time( 'mysql' ));
                        ...
                        do_action('inicis_escrowbank_confirm_no');
                    }
            ...
            function inicis_escrow_request_denyconfirm($posted){
                ...
                $iniescrow->startAction();
                 ...
                 if($resultCode == '00'){
                    $refunded_status = get_option('inicis_pg_inicis_stdescrowbank_order_status_after_refund');
                    $orderinfo->update_status( $refunded_status );  //환불처리완료 상태로 변경
            ...
            function inicis_escrow_get_order($posted){
                ...
                $payment_method = get_post_meta($_POST['postid'], '_payment_method', true);
                $mid = get_option('inicis_pg_escrow_merchant_id');
                $tid = get_post_meta($_POST['postid'], '_inicis_paymethod_tid', true);
		...
		if ( defined('DOING_AJAX') ) {
			$ajax_requests = array('payment_form_inicis_stdescrow_bank', 'refund_request_inicis_stdescrow_bank', 'inicis_stdescrow_bank_order_cancelled');
			if( in_array( $_REQUEST['action'], $ajax_requests ) ){
				new WC_Gateway_Inicis_StdEscrow_bank();
			}
		}
	}
