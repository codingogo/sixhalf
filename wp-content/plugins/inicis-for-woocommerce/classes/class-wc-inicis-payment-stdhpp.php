	...
	if ( ! class_exists( 'WC_Gateway_Inicis_StdHpp' ) ) {
		...
		class WC_Gateway_Inicis_StdHpp extends WC_Gateway_Inicis {
			public function __construct() {
				...
				$this->method_title = __('휴대폰 소액결제 (웹표준)', 'inicis_payment');
				...
				$settings = inicis_pg_get_setting( 'stdhpp' );
//				$this->init_settings();
            ...
            function thankyou_page( $order_id ) {
                echo __('<div id="inicis_thankyou_text"><p>휴대폰 소액결제로 결제되었습니다. 감사합니다.</p></div>', 'inicis_payment');
            }
...
//				unset( $this->form_fields['signkey'] );
		...
		if ( defined('DOING_AJAX') ) {
			$ajax_requests = array('payment_form_inicis_stdhpp', 'refund_request_inicis_stdhpp');
			if( in_array( $_REQUEST['action'], $ajax_requests ) ){
				new WC_Gateway_Inicis_StdHpp();
			}
		}	
	}
