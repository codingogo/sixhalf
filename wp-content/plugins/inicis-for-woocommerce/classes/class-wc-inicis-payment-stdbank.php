	...
	if ( ! class_exists( 'WC_Gateway_Inicis_Stdbank' ) ) {
		...
		class WC_Gateway_Inicis_Stdbank extends WC_Gateway_Inicis {
			public function __construct() {
				...
				$this->method_title = __('PC 실시간계좌이체(웹표준)', 'inicis_payment');
				...
				$settings = inicis_pg_get_setting( 'stdbank' );
//				$this->init_settings();
            ...
            function thankyou_page( $order_id ) {
                echo __('<div id="inicis_thankyou_text"><p>실시간 계좌이체로 결제되었습니다. 감사합니다.</p></div>', 'inicis_payment');
            }
		...
		if ( defined('DOING_AJAX') ) {
			$ajax_requests = array('payment_form_inicis_stdbank', 'refund_request_inicis_stdbank');
			if( in_array( $_REQUEST['action'], $ajax_requests ) ){
				new WC_Gateway_Inicis_Stdbank();
			}
		}	
	}
