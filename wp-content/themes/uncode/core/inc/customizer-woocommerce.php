...
function uncode_get_cart_items() {
	...
	if (  $articles > 0 ) {
		...
		$cart .= '<li><span>' . esc_html__('장바구니가 비었습니다.','uncode') . '</span></li>';
	}
...
function uncode_add_cart_in_menu($woo_icon, $woo_cart_class) {
	...
	$cart_container = '<ul role="menu" class="drop-menu sm-nowrap cart_list product_list_widget uncode-cart-dropdown">'.((isset($get_cart_items['cart']) && $get_cart_items['cart'] !=='') ? $get_cart_items['cart'] : '<li><span>' . esc_html__('장바구니가 비었습니다.','uncode') . '</span></li>').'</ul>';
...
function woocommerce_button_proceed_to_checkout() {
	...
	?>
	<a href="<?php echo esc_url($checkout_url); ?>" class="checkout-button btn btn-default alt wc-forward"><?php esc_html_e( '주문서 작성 ', 'woocommerce' ); ?></a>
	<?php
}
...
function uncode_woocommerce_available_variation( $variations ) {
	...
	if (isset($get_media_url) && $get_media_url !== '') {
		...
		if (isset($get_media_id) && $get_media_id !== '') {
			...
			$image_resized = uncode_resize_image($image_attributes->id, $image_attributes->guid, $image_attributes->path, $image_metavalues['width'], $image_metavalues['height'], 5, ($crop ? 5 / $thumb_ratio : null), $crop);
